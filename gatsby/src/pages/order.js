...
import calulateOrderTotal from '../utils/calculateOrderTotal'
  ...
  return (
    <>
      ...
      <OrderStyles onSubmit={submitOrder} >
          ...
          {pizzas.map((pizza) => {
            return (
              <MenuItemStyles key={pizza.id}>
                ...
                <div>
                  {['S', 'M', 'L'].map(size => (
                    <button key={size} onClick={() => addToOrder({
                      id: pizza.id,
                      size,
                    })} type="button">
        ...
        <fieldset disabled={loading}>  
        <h3>Your total is {formatMoney(calulateOrderTotal(order, pizzas))}</h3>
